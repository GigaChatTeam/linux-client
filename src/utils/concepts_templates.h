...
template <typename T>
concept json_type =
    is_one_of<T, bool, int, qint64, double, QString, QJsonObject, QJsonArray>;
...
std::optional<T> getJsonSafe(QString key, const QJsonObject &o)
{
	...
	if constexpr (std::is_arithmetic_v<T>)
		...
		return v.isString() ? std::make_optional<T>(v.toString())
				    : std::nullopt;
	else if constexpr (std::is_same_v<T, QJsonArray>)
		return v.isArray() ? std::make_optional<T>(v.toArray())
				   : std::nullopt;
	else if constexpr (std::is_same_v<T, QJsonObject>)
		return v.isObject() ? std::make_optional<T>(v.toObject())
				    : std::nullopt;
	return std::nullopt;
}
