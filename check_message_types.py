def find(data: list[str], start: int, elem: str):
    for index, e in enumerate(data[start:]):
...
for i, j in zip(enum_entries, array_entries):
    if i.strip() != j.strip():
        print(("DIFF:\n" +
               ...
               "\x1b[0m") % ("ENUM", "ARRAY", i.strip(), j.strip()))
        same = False

if same:
    print("\x1b[93mNo differences spotted!\x1b[0m\n");
else:
    print("\x1b[1;91mDifferences spotted!\x1b[0m")
